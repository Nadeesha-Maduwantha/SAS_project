...
export default function CustomUserLeftNavBar({ topOffset = 57 }: { topOffset?: number }) {
  const router   = useRouter();
  const pathname = usePathname();
  const { expanded, toggle, expand, collapse } = useNav();
  // Cover Access only shows up for a custom user type once an admin has
  // switched it on for that type (System Settings -> User Types) — the
  // same check services/cover_access.py enforces server-side.
  ...
  return (
    <div ref={navRef} className="admin-nav-container expanded" style={containerStyle}>
      ...
      {/* Dashboard */}
      <button
        className={`nav-dashboard-btn ${active('/custom_user/dashboard') ? 'active' : ''}`}
        onClick={() => go('/custom_user/dashboard')}
      >
      ...
      <div className="nav-section">
        <button
          className={`nav-section-header ${active('/custom_user/shipments') ? 'active' : ''}`}
          onClick={() => go('/custom_user/shipments')}
        >
      ...
      <div className="nav-section">
        <button
          className={`nav-section-header ${active('/custom_user/alerts') ? 'active' : ''}`}
          onClick={() => go('/custom_user/alerts')}
        >
      ...
      >
        <NavItem
          ...
          isActive={active('/custom_user/current_milestone')}
          onClick={() => go('/custom_user/current_milestone')}
        />
      </Section>
      ...
      >
        <NavItem
          label="My Tables"
          isActive={active('/custom_user/custom_tables')}
          onClick={() => go('/custom_user/custom_tables')}
        />
      </Section>
      ...
      >
        <NavItem
          label="My Profile"
          isActive={active('/custom_user/profile')}
          onClick={() => go('/custom_user/profile')}
        />
        {coverEligible && (
          <NavItem
            label="Cover Access"
            isActive={active('/custom_user/cover')}
            onClick={() => go('/custom_user/cover')}
          />
        )}
      </Section>
    </div>
