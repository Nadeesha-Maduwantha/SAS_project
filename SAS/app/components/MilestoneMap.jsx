...
export default function MilestoneMap({ milestone, allMilestones, currentIndex }) {
  ...
  const drawAll = (L, map, milestones, activeIdx) => {
    ...
    polylineRef.current = L.polyline(latlngs, {
      color: "#3b82f6", weight: 2, opacity: 0.5, dashArray: "6 5",
    }).addTo(map);
    ...
    milestones.forEach((m, i) => {
      ...
      const icon = L.divIcon({
        className: "",
        html: isActive
          ? `<div style="width:18px;height:18px;background:${color};border:3px solid white;border-radius:50%;box-shadow:0 0 0 4px ${color}55,0 2px 8px rgba(0,0,0,0.4);"></div>`
          : `<div style="width:10px;height:10px;background:${color};border:2px solid rgba(255,255,255,0.6);border-radius:50%;opacity:0.85;"></div>`,
  ...
  useEffect(() => {
    ...
    if (mapElement && mapElement._leaflet_id) mapElement._leaflet_id = null;
    if (!mapElement) return;

    // ✅ Dynamic import — only runs on client, never on server
    Promise.all([
      import("leaflet"),
    ...
    return () => {
      ...
      if (mapElement) delete mapElement._leaflet_id;
      markersRef.current = [];
  ...
  useEffect(() => {
    ...
    drawAll(L, map, allMilestones, currentIndex);
    map.flyTo(
      [milestone.location.lat, milestone.location.lng], 5,
