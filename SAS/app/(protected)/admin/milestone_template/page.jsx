...
export default function MilestoneTemplatePage() {
  ...
  const loadTemplate = async () => {
    try {
      setError(null);
      const res    = await fetch(`http://127.0.0.1:5000/api/templates/${templateId}`, { headers: authHeaders() });
  ...
  const handlePromote = async (slot) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:5000/api/milestone-library/promote`, {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (res.ok) { setShowDelete(false); router.push("/admin/milestone_templates_list"); }
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: T.pageBg, fontFamily: T.font, color: T.gray900, padding: "32px 32px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "22px" }}>
          <span onClick={() => router.push("/admin/milestone_templates_list")}
              ...
              {newCopyId && (
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
                    <button onClick={() => router.push(`/admin/milestone_template?id=${newCopyId}`)}
