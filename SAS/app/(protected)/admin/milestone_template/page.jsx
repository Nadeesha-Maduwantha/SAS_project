...
export default function MilestoneTemplatePage() {
  ...
  const handleSaveCopy = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/templates/${tmpl.id}/copy`, {
        method: "POST",
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/templates/${tmpl.id}`, { method: "DELETE" });
      if (response.ok) {
        setShowDelete(false);
