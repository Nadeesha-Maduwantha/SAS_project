...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function OperationsDigestLogPage() {
    ...
    const fetchHistory = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`${BACKEND_BASE_URL}/api/operations-digest/history?limit=200`, {
                headers: authHeaders(),
            });
    ...
    const handleSendNow = async () => {
        const confirmed = window.confirm(
            'This will immediately email every operation user with overdue or upcoming-due milestones assigned to them. Continue?'
        );
        if (!confirmed) return;
        ...
        setSendResult(null);
        try {
            const response = await fetch(`${BACKEND_BASE_URL}/api/operations-digest/run`, {
                method: 'POST',
    ...
    const filtered = rows.filter((r) => {
        ...
        const matchSearch = search === '' ||
            r.operations_email.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#1a1a2e', letterSpacing: '-0.4px' }}>Operations Digest Emails</h1>
                    <p style={{ fontSize: '13.5px', color: '#6b7280', marginTop: '4px' }}>Automatic overdue and upcoming-reminder emails the system has sent to operation users.</p>
                </div>
            ...
            <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 20px', borderBottom: '1px solid #f0f0f0', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '7px 12px', width: '220px' }}>
                        ...
                        <input placeholder="Search email or subject..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ border: 'none', background: 'transparent', outline: 'none', fontSize: '13px', width: '100%', color: '#374151' }} />
                    </div>
                </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: '#f9fafb', borderBottom: '1px solid #f0f0f0' }}>
                                ...
                                <th style={thStyle}>OPERATION USER</th>
                        ...
                        <tbody>
                            {filtered.map((row, idx) => (
                                ...
                                >
                                    ...
                                    <td style={{ ...tdStyle, fontSize: '13px', color: '#374151', fontWeight: 500 }}>{row.operations_email}</td>
                            ...
                            {filtered.length === 0 && (
                                <tr>
                                    <td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>
                                        No operations digest emails have been sent yet.
                                    </td>
                                </tr>
                            )}
                        </tbody>
