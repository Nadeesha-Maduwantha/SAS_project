          ...
          {data.length === 0 ? (
            ...
            data.map((t) => (
              <tr key={t.id} className="hover:bg-gray-50 transition-colors group">
                ...
                {/* Actions — View and Edit only, duplicate removed */}
                <td className="px-5 py-4">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">

                    {/* View — opens template in view mode */}
                    <Link
                      href={`${basePath}?id=${t.id}`}
                    ...
                    {/* Edit — opens template with edit mode pre-activated */}
                    <Link
                      href={`${basePath}?id=${t.id}&edit=true`}
