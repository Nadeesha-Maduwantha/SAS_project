...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function SalesDigestLogPage() {
    ...
    const fetchHistory = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`${BACKEND_BASE_URL}/api/sales-digest/history?limit=200`, {
                headers: authHeaders(),
            });
    ...
    const handleSendNow = async () => {
        const confirmed = window.confirm(
            'This will immediately email every sales user with overdue or upcoming-due milestones. Continue?'
        );
        if (!confirmed) return;
        ...
        setSendResult(null);
        try {
            const response = await fetch(`${BACKEND_BASE_URL}/api/sales-digest/run`, {
                method: 'POST',
    ...
    const filtered = rows.filter((r) => {
        ...
        const matchSearch = search === '' ||
            r.sales_email.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#1a1a2e', letterSpacing: '-0.4px' }}>Sales Digest Emails</h1>
                    <p style={{ fontSize: '13.5px', color: '#6b7280', marginTop: '4px' }}>Automatic overdue and upcoming-reminder emails the system has sent to sales users.</p>
                </div>
                        ...
                        <tbody>
                            {filtered.map((row, idx) => (
                                ...
                                >
                                    ...
                                    <td style={{ ...tdStyle, fontSize: '13px', color: '#374151', fontWeight: 500 }}>{row.sales_email}</td>
                            ...
                            {filtered.length === 0 && (
                                <tr>
                                    <td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>
                                        No sales digest emails have been sent yet.
                                    </td>
                                </tr>
                            )}
                        </tbody>
