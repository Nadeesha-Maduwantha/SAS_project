...
import { useEmailEligibility } from '@/lib/hooks/useCoverEligibility';
...
export default function AlertDashboardPage() {
    ...
    // Custom user types (System Settings -> User Types) only get the manual
    // "Email" compose action when an admin has switched it on for that type.
    const { eligible: emailEligible } = useEmailEligibility();
    ...
    const fetchAlerts = async () => {
        ...
        try {
            ...
            // Custom user types are scoped generically by role, matched against
            // that type's own field mapping (services/scope.py) — not the
            // sales_user_email match the plain ?email= param triggers on the
            // backend. See routes/alerts.py get_alerts()'s additive ?role= branch.
            const url = userEmail
                ? `${BACKEND_BASE_URL}/api/alerts?email=${encodeURIComponent(userEmail)}&role=${encodeURIComponent(role || '')}`
    ...
    useEffect(() => { fetchAlerts(); }, [email, role]);
    ...
    return (
        <div>
            ...
            <AlertDetailsModal
                ...
                onEmailClick={emailEligible ? openCompose : undefined}
            />
            <EmailComposeModal
                isOpen={composeOpen}
                                    ...
                                    >
                                        ...
                                        <td style={tdStyle} onClick={(e) => e.stopPropagation()}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                ...
                                                {emailEligible && <ActionBtn icon={<Mail size={14} />} title="Email" onClick={() => openCompose(toAlertData(alert))} />}
                                            </div>
                                        </td>
                                    </tr>
                                ))}
