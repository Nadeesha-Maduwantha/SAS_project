...
export default function MilestonePage() {
  ...
  const USER_ROLE = "sales"; // replace with real role from session later
  // Custom user types (System Settings -> User Types) only get this compose
  // button when an admin has switched "Can email clients directly" on.
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:5000/api/shipments/${shipmentId}`)
      .then(r => r.json())
      .then(result => {
      ...
      <div className="w-72 flex-shrink-0 border-l border-gray-200 bg-white overflow-y-auto flex flex-col">
        <div className="px-4 pt-5 pb-4 border-b border-gray-100">
          {milestone?.status === "completed" ? (
            ...
            <div className="w-full px-4 py-3 rounded-xl text-xs text-center bg-gray-50 text-gray-400 border border-gray-200">
              Emailing isn't enabled for your account type.
            </div>
          ) : (
            <button
