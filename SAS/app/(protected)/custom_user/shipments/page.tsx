...
import { getShipmentsByRole } from '@/lib/services/shipment.service'
...
export default function CustomUserShipmentsPage() {
  ...
  const { email, name, role, department } = useAuth()
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        // Custom user types are scoped generically by role (services/scope.py),
        // not by a sales-specific column — see getShipmentsByRole.
        const data = await getShipmentsByRole(role, email, department)
        setShipments(data)
      } catch (err) {
    ...
    fetchData()
  }, [email, role, department]) // re-fetches if identity changes when real auth is connected
  ...
  function renderAction(shipment: Shipment) {
    ...
    return (
      <button
        onClick={(e) => {
          e.stopPropagation()
          router.push(`/custom_user/shipments/${shipment.id}/action`)
        }}
